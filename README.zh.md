...
如果你使用 AI 中转站，并且中转站开放了查询额度的 API 接口，`codex-hud` 可以实时显示该中转站的已用额度和剩余额度。

开启后，`codex-hud` 会从本机环境变量读取 `api_key_env` 指定的 key，并只请求你在 `usage_url` 中配置的额度查询接口。HUD 不会显示、保存或上报你的 key，也不会发送你的会话内容、提示词、代码、支付信息或其他隐私数据；除你明确配置的额度查询接口外，额度显示功能本身不会额外联网。

如果中转站没有提供额度查询接口，或接口暂时不可用，`codex-hud` 会继续正常显示其他 HUD 信息，只是不展示实时额度。
